# ==============================================================================
# PROJECT: Command Center - Streamlined Final Version
...
# La clé API Météo n'est plus nécessaire.
...
class App(customtkinter.CTk):
        ...
        try:
            ...
            for event in reversed(live_data['features']):
                if event['id'] not in self.processed_event_ids:
                    new_event_count += 1
                    self.processed_event_ids.add(event['id'])
                    self.process_single_seismic_event(event)
    ...
    def process_single_seismic_event(self, event, is_playback=False):
        ...
        features_anomaly = pd.DataFrame([[depth, mag]], columns=['Depth', 'Magnitude'])
        ...
        # Logique Tsunami à double facteur (probabilité + magnitude)
        TSUNAMI_PROBA_THRESHOLD = 0.70 
        TSUNAMI_MAG_THRESHOLD = 7.5
        is_tsunami_risk = (tsunami_proba > TSUNAMI_PROBA_THRESHOLD and mag > TSUNAMI_MAG_THRESHOLD)
        ...
        if anomaly_score < -0.01:
            estimated_yield = self.estimate_yield(mag)
        ...
        details = {'place': properties.get('place'), 'time': datetime.fromtimestamp(properties['time']/1000).strftime('%Y-%m-%d %H:%M:%S'), 'mag': f"{mag:.1f}", 'depth': f"{depth:.1f}", 'status': status, 'url': properties.get('url'), 'yield': yield_info.replace('\n', '')}
        ...
        self.seismic_log.insert('', 'end', values=log_entry, tags=(tag,))
    ...
    def pulse_marker(self, marker, steps=10):
        try:
            if steps>0:
                current_color = marker.marker_color_circle
                next_color = "white" if current_color != "white" else marker.final_color
                marker.set_marker_color_circle(next_color)
                self.after(150, lambda: self.pulse_marker(marker, steps - 1))
    ...
    def setup_solar_tab(self):
        summary_frame = customtkinter.CTkFrame(self.solar_tab, fg_color="transparent")
        ...
        self.solar_log.heading('Time', text='Heure Début (UTC)'); self.solar_log.column('Time', width=200)
        self.solar_log.heading('Class', text='Classe'); self.solar_log.column('Class', width=150)
        self.solar_log.heading('Region', text='Région Solaire'); self.solar_log.column('Region', width=150)
        self.solar_log.heading('Geo-effective', text='Menace Terre?'); self.solar_log.column('Geo-effective', width=150, anchor='center')
        self.solar_log.pack(fill="both", expand=True, padx=5, pady=5)
        self.solar_log.tag_configure('geoeffective_x', foreground='red', font=('Calibri', 12, 'bold'))
        self.solar_log.tag_configure('geoeffective_m', foreground='orange', font=('Calibri', 11, 'bold'))
    ...
    def update_solar_data(self):
        solar_update_interval=900000
        if self.is_playback_mode: self.after(solar_update_interval, self.update_solar_data); return
        try:
            end_date, start_date = datetime.utcnow(), datetime.utcnow() - timedelta(days=2)
            url=f"https://api.nasa.gov/DONKI/FLR?startDate={start_date.strftime('%Y-%m-%d')}&endDate={end_date.strftime('%Y-%m-%d')}&api_key=DEMO_KEY"
            flares=requests.get(url, timeout=10).json()
            self.solar_log.delete(*self.solar_log.get_children())
            ...
            for flare in reversed(flares):
                ...
                if is_geo and (class_type.startswith('X') or class_type.startswith('M')):
                    geo_effective_count += 1
                    tag = 'geoeffective_x' if class_type.startswith('X') else 'geoeffective_m'
        ...
        except requests.exceptions.RequestException as e: print(f"Erreur API Solaire: {e}")
        self.after(solar_update_interval, self.update_solar_data)
    ...
    def update_asteroid_data(self):
        asteroid_update_interval=3600000
        if self.is_playback_mode: self.after(asteroid_update_interval, self.update_asteroid_data); return
        try:
            ...
            closest_approach_name, largest_diameter_name = "--", "--"
            closest_dist, largest_diam, high_risk_count = float('inf'), 0, 0
            if int(asteroids.get('count', 0)) > 0:
                for data in asteroids['data']:
                    ...
                    dist_km=float(data_map.get('dist', 0)) * 149597870.7
        ...
        self.after(asteroid_update_interval, self.update_asteroid_data)
